...
# Connect DB stuff
conn = psycopg2.connect("dbname='assessment' user='iugo' host='localhost' password='postgres'")
curs = conn.cursor()
...
data.seek(0)
a = curs.copy_from(data, 'crash_report', columns=('user_id', 'timestamp', 'message'), sep=',')
connection.close()
